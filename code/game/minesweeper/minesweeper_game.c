...
void doAIAction (void) {
    if (!ms->firstClick) {
        int randomRow = (int)(randomU32() % GRID_HEIGHT);
        int randomCol = (int)(randomU32() % GRID_WIDTH);
        ...
        b32 makeMistake = randomF32() < 0.15f;
        b32 fixWrongFlag = randomF32() < 0.5f;

        mine_cell_ptr_list wrongFlagCells = mine_cell_ptr_listInit(msScratchMemory, GRID_WIDTH * GRID_HEIGHT);
            ...
            if (!didAction) {
                ...
                if (unsolvedCells.numValues > 0) {
                    ...
                    if (cell->adjCells.numValues > 0) {
                        ms->aiAction = AI_ACTION_REVEAL_CELL;
                        ms->targetCellRow = cell->adjCells.values[0]->row;
                        ms->targetCellCol = cell->adjCells.values[0]->col;
        ...
        switch (ms->aiState) {
            ...
            case AI_STATE_THINK_DELAY: {
                ...
                if (waitDone) {
                    ms->aiState = AI_STATE_THINKING;
                    ms->aiTimer = 0.0f;
                    ms->moveDuration = 0.75f * randomF32() * 1.5f;
                }
            } break;
        }
    }
