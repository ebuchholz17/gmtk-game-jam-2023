...
#include "minesweeper/minesweeper_game.c"
...
UPDATE_GNG_GAME(updateGNGGame) {
    ...
    if (state->assetMan.allFilesLoaded) {
        ...
        if (!state->msState.initialized) {
            initMinesweeper(&state->msState, &state->memory);
        }
        ...
        if (!(platAPI.windowWidth == 0 && platAPI.windowHeight ==0)) {
            ...
            // draw
            drawMinesweeper(platAPI, gameScale, &scratchMemory);
